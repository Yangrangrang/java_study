...
public class CommandManagerV2 implements CommandManager {
    ...
    public void execute(String totalMessages, Session session) throws IOException {
        ...
        if (totalMessages.startsWith("/join")) {
            String[] split = totalMessages.split(DELIMITER);
            String username = split[1];
            session.setUsername(username);
            sessionManager.sendAll(username + "님이 입장했습니다.");
        } else if (totalMessages.startsWith("/message")) {
            // 클라이언트 전체에게 문자 보내기
            String[] split = totalMessages.split(DELIMITER);
            String message = split[1];
            sessionManager.sendAll("[" + session.getUsername() + "] " + message);
        } else if (totalMessages.startsWith("/change")) {
            String[] split = totalMessages.split(DELIMITER);
            String changeName = split[1];
            sessionManager.sendAll(session.getUsername() + "님이 " + changeName + "로 이름을 변경했습니다.");
            session.setUsername(changeName);
        } else if (totalMessages.startsWith("/users")) {
        ...
        } else if (totalMessages.equals("/exit")) {
            throw new IOException("exit");
        } else {
            session.send("처리할 수 없는 명령어 입니다. " + totalMessages);
        }
    }
}
